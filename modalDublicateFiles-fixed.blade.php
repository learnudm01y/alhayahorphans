    ...
    fetch(`/api/duplicate-files/summary?session_id=${sessionId}`, {
        ...
        .then(data => {
            ...
            if (data.success && data.data) {
                ...
                // إظهار رابط التحميل والحذف حتى في حالة عدم وجود استجابة صحيحة (لأغراض الاختبار)
                const finalDownloadLink = document.getElementById('downloadDuplicatesLink');
                if (finalDownloadLink) {
                    const baseUrl = window.location.protocol + '//' + window.location.host;
                    const downloadUrl = `${baseUrl}/api/duplicate-files/download?session_id=${sessionId}`;
                    ...
                    finalDownloadLink.download = `duplicate_files_${sessionId}.zip`;
                    finalDownloadLink.style.display = 'inline-block';
                    console.log('🔗 تم إعداد رابط التنزيل (حالة طوارئ):', downloadUrl);
                }
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('🚀 تحميل Modal الملفات المكررة - النسخة النهائية والنظيفة');
