...
const SyncService = {
    ...
    dbVersion: 8, // تحديث الإصدار لحل مشاكل التوافق
    db: null,
    ...
    async performInitialSync() {
        let initialNotifId = null;
        try {
            this.syncInProgress = true;

            // إرسال إشعار بدء المزامنة (نحتفظ بالمعرف ليتم إخفاؤه عند الانتهاء)
            initialNotifId = await this.sendNotification('جاري المزامنة...', 'يتم تحميل البيانات الأساسية', 0);
    ...
    async performFullSync(onProgress = null) {
        ...
        try {
            ...
            // إرسال إشعار بدء المزامنة (نحتفظ بالمعرف ليتم إخفاؤه عند الانتهاء)
            fullNotifId = await this.sendNotification('جاري مزامنة الكفالات...', 'يتم جلب البيانات من الخادم', 0);
    ...
    async uploadPendingChanges() {
        ...
        uploadNotifId = await this.sendNotification('جاري رفع التعديلات...', `${totalItems} تعديل معلق`, 0);
    ...
    async saveFile(sponsorshipId, fileData, fileName, fileType) {
        console.log('🔥 ENTER saveFile - ID:', sponsorshipId, 'File:', fileName);

        if (!window.Capacitor || !window.Capacitor.Plugins || !window.Capacitor.Plugins.UploadService) {
            console.error('❌ UploadService NOT AVAILABLE!');
            throw new Error('UploadService غير متاح');
        }

        console.log('✅ UploadService available - calling addFileToQueue...');
        ...
        const uploadData = {
            ...
            apiUrl: SyncService.baseUrl + '/mobile/upload-file'
        };

        console.log('📤 Calling Java Plugin with:', uploadData.fileName, uploadData.apiUrl);

        try {
            const response = await window.Capacitor.Plugins.UploadService.addFileToQueue(uploadData);
            console.log('✅ Java response:', response);
            return response.fileId || 0;
        } catch (error) {
            console.error('❌ Java call failed:', error);
            throw error;
        }
    },
