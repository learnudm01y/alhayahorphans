...
const SyncService = {
    ...
    async performFullSync(onProgress = null) {
        try {
            ...
            await this.showNotificationComplete(`✅ تم تنزيل ${totalDownloaded} كفالة بنجاح`);
    ...
    async uploadPendingChanges() {
        ...
        // تحديث Progress Bar في إشعار Android
        await this.updateNotificationProgress(0, totalItems, 'جاري رفع التعديلات...');
        ...
        for (const change of pending) {
            try {
                ...
                // تحديث Progress Bar في إشعار Android
                await this.updateNotificationProgress(processedItems, totalItems, `تم رفع ${processedItems} من ${totalItems}`);
                ...
                await this.updateNotificationProgress(processedItems, totalItems, `خطأ في ${processedItems} - متابعة...`);
            }
        }
        ...
        if (results.failed === 0) {
            ...
            await this.showNotificationComplete(`✅ تم رفع ${results.success} تعديل بنجاح`);
        } else {
            await this.sendNotification('⚠️ اكتمل الرفع مع أخطاء', `نجح: ${results.success} | فشل: ${results.failed}`);
            await this.showNotificationComplete(`⚠️ نجح: ${results.success} | فشل: ${results.failed}`);
        }
    ...
    async initAutoUpload() {
        ...
        window.addEventListener('online', () => {
            ...
            // بدء الرفع التلقائي عند عودة الاتصال
        ...
        // رفع التحديثات المعلقة عند البدء إذا كان الإنترنت متصلاً
        if (this.networkStatus) {
            setTimeout(() => {
                this.autoUploadPendingChanges();
            }, 2000); // تأخير قصير للتأكد من اكتمال التهيئة
        }
...
if (typeof window !== 'undefined') {
    ...
    document.addEventListener('pause', async () => {
        ...
        if (SyncService.syncInProgress || SyncService.uploadInProgress || SyncService.autoUploadInProgress) {
    ...
    document.addEventListener('resume', () => {
        ...
        // تحقق من طابور الرفع عند العودة
