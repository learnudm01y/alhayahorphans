...
public class ChunkedUploadWorker extends Worker {
    ...
    private int getUploadWorkers() {
        try {
            int w = getApplicationContext().getSharedPreferences("upload_prefs", Context.MODE_PRIVATE).getInt("upload_workers", 1);
    ...
    public Result doWork() {
        ...
        try {
            ...
            Log.i(TAG, "Upload workers: " + workers);
            ...
            // Parallel mode: multiple files concurrently
            final AtomicInteger processed = new AtomicInteger(0);
            final AtomicInteger success = new AtomicInteger(0);
            final AtomicBoolean sawFailure = new AtomicBoolean(false);
            final AtomicBoolean yielded = new AtomicBoolean(false);
                ...
                executor.submit(() -> {
                    try {
                        while (true) {
                            ...
                            if (budgetExhausted()) { yielded.set(true); break; }
                            if (processed.get() >= MAX_FILES_PER_RUN) break;
                            UploadDatabaseHelper.UploadItem nextFile;
                            synchronized (dbHelper) {
                                nextFile = dbHelper.claimNextPendingFile();
                            }
                            if (nextFile == null) break;
                            int cur = processed.incrementAndGet();
                            Log.d(TAG, "📤 [Worker] معالجة الملف #" + cur + ": " + nextFile.fileName);
                            ...
                            if (uploadError2 == null) {
                                ...
                                sawFailure.set(true);
                                if (!InternetUtils.hasAnyNetwork(getApplicationContext())) {
                                    synchronized (dbHelper) { dbHelper.updateFileStatus(nextFile.id, UploadDatabaseHelper.STATUS_PENDING, "انقطعت الشبكة"); }
                                    stopped.set(true);
                                    break;
                                }
                                synchronized (dbHelper) {
                                ...
                                if (sawFailure.get()) {
                                    // let other workers finish current files but don't start new ones
                                    break;
                                }
                            }
                        }
            ...
            try (Response response = httpClient.newCall(request).execute()) {
                ...
                for (int i = 0; i < dataArray.length(); i++) {
                    ...
                    if ("completed".equals(status)) {
                        ...
                        // Doc/13: فهرسة فورية بعد نجاح الرفع بدل انتظار الدورة القادمة
                        try {
                            org.json.JSONObject entry = new org.json.JSONObject();
                            entry.put("file_id", String.valueOf(item.id));
                            entry.put("stored_file_name", item.fileName);
                            entry.put("file_path", item.filePath != null ? item.filePath : "");
                            entry.put("source", "chunked_upload");
                            entry.put("source_id", item.id);
                            entry.put("status", "active");
                            com.aso.app.v4.admin_offline.AdminOfflineDataStoreV4
                                .upsertFileIndexEntry(getApplicationContext(), entry);
