...
public class RelatedDataDatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        // جدول additional_deceased (متوفون إضافيون)
        db.execSQL("CREATE TABLE additional_deceased ("
                + "id INTEGER PRIMARY KEY, "
                + "re_file_id TEXT, "
                + "person_id TEXT, "
                + "first_name TEXT, "
                ...
                + "updated_at INTEGER"
                + ")");
        db.execSQL("CREATE INDEX idx_add_dead_file ON additional_deceased(re_file_id)");
        db.execSQL("CREATE INDEX idx_add_dead_person ON additional_deceased(person_id)");
            ...
            for (int i = 0; i < records.length(); i++) {
                ...
                switch (tableName) {
                    ...
                    case "additional_deceased":
                        ...
                        cv.put("third_name", obj.optString("third_name", ""));
                        cv.put("last_name", obj.optString("last_name", ""));
                        cv.put("relationship", obj.optString("relationship", ""));
                        cv.put("death_date", obj.optString("death_date", ""));
                        cv.put("death_reason", obj.optInt("death_reason", 0));
                        String addSearch = (obj.optString("first_name", "") + " "
                                + obj.optString("last_name", "") + " "
                                + obj.optString("person_id", "")).toLowerCase().trim();
                        cv.put("search_text", addSearch);
                        break;
                }
    ...
    /**
     * البحث برقم التسجيل في جدول re_people (جلب كل الأفراد)
     */
    public JSONArray getRePeopleByRegistration(String registrationId) {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM re_people WHERE registration_id = ?",
        ...
        try {
            ...
            Log.e(TAG, "getRePeopleByRegistration error", e);
        } finally {
            cursor.close();
        }
        return results;
    ...
    /**
     * جلب المتوفين الإضافيين برقم الملف
     */
    public JSONArray getAdditionalDeceasedByRegistration(String registrationId) {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM additional_deceased WHERE re_file_id = ?",
                new String[]{registrationId});
        JSONArray results = new JSONArray();
        try {
            if (cursor.moveToFirst()) {
            ...
            Log.e(TAG, "getAdditionalDeceasedByRegistration error", e);
        } finally {
            cursor.close();
        }
        return results;
