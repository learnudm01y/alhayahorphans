                        ...
                        for (int i = 0; i < dataArray.length(); i++) {
                            ...
                            if (localItem != null) {
                                if ("completed".equals(status)) {
                                    ...
                                    // Doc/13: فهرسة فورية بعد نجاح الرفع
                                    try {
                                        org.json.JSONObject entry = new org.json.JSONObject();
                                        entry.put("file_id", String.valueOf(localItem.id));
                                        entry.put("stored_file_name", localItem.fileName);
                                        entry.put("file_path", localItem.filePath != null ? localItem.filePath : "");
                                        entry.put("source", "chunked_upload");
                                        entry.put("source_id", localItem.id);
