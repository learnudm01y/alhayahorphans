...
public class AutoUploadApplication extends Application {
    ...
    public void onCreate() {
        ...
        // ── v4 Dual-Run: schedule unified sync alongside v3 (additive only).
        // Unique work "unified_sync_v4" + KEEP prevents a second concurrent worker.
        // v3 jobs above are intentionally left untouched until Phase 7 cutover.
        try {
            com.aso.app.v4.sync.SyncSchedulerV4.schedulePeriodicSync(this);
            Log.i(TAG, "v4 unified sync scheduled (Dual-Run with v3)");
        } catch (Throwable t) {
            Log.e(TAG, "v4 sync scheduling failed (v3 unaffected)", t);
        }

        // ── Phase 7: apply cached legacy_sync_enabled flag (cancels v3 unique
        // works at runtime if server already cut over; no code is deleted).
        try {
            com.aso.app.v4.sync.RemoteConfigManagerV4.applyCachedFlag(this);
        } catch (Throwable t) {
            Log.e(TAG, "remote config apply failed (v3 unaffected)", t);
        }
    }
}
