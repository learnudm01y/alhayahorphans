...
public class UploadServicePlugin extends Plugin {
    ...
    @PluginMethod
    public void setUploadWorkers(PluginCall call) {
        try {
            Integer workers = call.getInt("workers", 1);
            if (workers == null) workers = 1;
            workers = Math.min(Math.max(workers, 1), 4);
            getContext().getSharedPreferences("upload_prefs", android.content.Context.MODE_PRIVATE)
                    .edit().putInt("upload_workers", workers).apply();
            JSObject ret = new JSObject();
            ret.put("success", true);
            ret.put("workers", workers);
            call.resolve(ret);
        } catch (Exception e) {
            call.reject("Failed to set workers: " + e.getMessage());
        }
    }
    ...
    public void getUploadWorkers(PluginCall call) {
        try {
            int workers = getContext().getSharedPreferences("upload_prefs", android.content.Context.MODE_PRIVATE)
                    .getInt("upload_workers", 1);
            JSObject ret = new JSObject();
            ret.put("workers", workers);
            call.resolve(ret);
        } catch (Exception e) {
            call.reject("Failed to get workers: " + e.getMessage());
        }
    }
