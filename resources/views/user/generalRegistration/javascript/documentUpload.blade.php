@push('scriptsCodeUserRegistration')
    ...
    <script>
        ...
        //     }
           ...
           window.handleAttachmentCancelAndCleanup = function(fileOrTaskId) {
            // 1. حذف المهمة من allDocs (يدعم تمرير الملف أو taskId)
            let found = false;
            if (typeof window.allDocs === 'object' && window.allDocs instanceof Map) {
                for (let [personKey, arr] of window.allDocs.entries()) {
                    let idx = -1;
                    // إذا كان fileOrTaskId نصاً، اعتبره taskId
                    if (typeof fileOrTaskId === 'string') {
                        idx = arr.findIndex(t => t.id === fileOrTaskId);
                    } else if (fileOrTaskId && typeof fileOrTaskId === 'object') {
                        // مقارنة بالملف أو خصائصه
                        idx = arr.findIndex(t =>
                            t.originalFile === fileOrTaskId ||
                            (t.originalFile &&
                                t.originalFile.name === fileOrTaskId.name &&
                                t.originalFile.size === fileOrTaskId.size &&
                                t.originalFile.type === fileOrTaskId.type)
                        );
                    }
                    if (idx !== -1) {
                        const task = arr[idx];
                        // تنظيف المؤقتات وURLs
                        if (task.timer) {
                            clearTimeout(task.timer);
                            task.timer = null;
                        }
