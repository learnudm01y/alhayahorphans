...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const idInput = document.getElementById('data_id_number') || document.querySelector('input[name="data_id_number"]');
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (saveBtn && idInput && passInput) {
            saveBtn.addEventListener('click', function(e) {
                ...
                if (window.PasswordCredential) {
                    if (confirm('سيتم حفظ رقم الهوية كاسم مستخدم وكلمة المرور في مدير كلمات المرور في جوجل. هل تريد المتابعة؟')) {
                        const cred = new window.PasswordCredential({
                            id: idInput.value,
                            password: passInput.value,
                            name: idInput.value
                        ...
                        delBtn.onclick = function() {
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    ...
                                    card.style.transition = 'filter 0.7s, opacity 0.7s, transform 0.7s, box-shadow 0.7s';
                                    card.style.filter = 'blur(7px) grayscale(0.7) contrast(1.5) drop-shadow(0 0 12px #b3e5fc)';
                                    card.style.opacity = '0';
                                    card.style.transform = 'scale(0.85) rotateZ(8deg) translateX(100px) skewY(6deg)';
                                    card.style.boxShadow = '0 0 60px 0 #00bcd44d, 0 0 0 2px #fff8';
