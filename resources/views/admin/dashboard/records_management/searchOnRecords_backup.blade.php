                    ...
                    <form id="searchForm" class="mb-4">
                        ...
                        <div class="row mt-4">
                            <div class="col-12">
                                ...
                                <button type="button" class="btn btn-success" id="exportResults" style="display: none;">
...
$(document).ready(function() {
    ...
    function displayResultsAsTable(data) {
        ...
        allRecords.forEach((record, index) => {
            ...
            const city = record.city_name || record.city || record.data_city || '-';
            const province = record.province_name || record.province || record.data_province || '-';
                ...
                '<tr class="table-row-hover">' +
                    ...
                    '<td class="text-center">' +
                        '<div class="btn-group btn-group-sm">' +
                            '<button class="btn btn-info btn-sm" onclick="showRecordDetails(' + JSON.stringify(record).replace(/"/g, '&quot;') + ')" title="عرض التفاصيل">' +
                                '<i class="fas fa-eye"></i>' +
                            '</button>' +
                            (record.view_url ? '<a href="' + record.view_url + '" class="btn btn-primary btn-sm" title="فتح السجل"><i class="fas fa-external-link-alt"></i></a>' : '') +
                        '</div>' +
                    '</td>' +
                '</tr>';
        });
    ...
    /*
     * عرض تفاصيل السجل في مودال
     */
    function showRecordDetails(record) {
        let modalContent = '';

        if (record.record_type === 'سجل رئيسي') {
            modalContent = formatMainRecordDetailsForModal(record);
        } else if (record.record_type === 'فرد أسرة') {
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            ...
                            ${record.view_url ? `<a href="${record.view_url}" class="btn btn-primary">فتح السجل الكامل</a>` : ''}
                        </div>
                    </div>
                </div>
            </div>
    ...
    function exportToExcel() {
        ...
        $('#resultsTableBody tr').each(function() {
            if ($(this).find('td').length > 1) { // تجاهل صف "لا توجد نتائج"
                const row = [];
                $(this).find('td').each(function(index) {
                    if (index < 9) { // فقط الأعمدة الأساسية (بدون الإجراءات)
        ...
        // إنشاء ملف Excel
        const headers = ['#', 'الاسم الكامل', 'رقم الهوية', 'رقم الجوال', 'العمر', 'الجنس', 'المدينة', 'المحافظة', 'حالة الطلب'];
        const ws = XLSX.utils.aoa_to_sheet([headers, ...tableData]);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, 'نتائج البحث');

        // تحميل الملف
        const filename = `نتائج_البحث_${new Date().toISOString().slice(0, 10)}.xlsx`;
    ...
    function exportSearchResults() {
        // يمكن تطويرها لاحقاً لتصدير النتائج إلى Excel أو PDF
        showInfo('سيتم تطوير ميزة التصدير قريباً');
    }
    ...
    function showRecordDetails(recordId) {
        console.log(`إظهار تفاصيل السجل ${recordId}`);

        // إرسال طلب للحصول على تفاصيل السجل الكاملة
        fetch(`{{ route('search.records') }}?record_id=${recordId}`, {
            method: 'GET',
            headers: {
        ...
        .then(data => {
            if (data.success && data.data && data.data.main_records && data.data.main_records.length > 0) {
                const record = data.data.main_records[0];
                showDetailedModal(record);
            } else {
                showErrorModal('عذراً، لم يتم العثور على تفاصيل السجل');
            }
        })
        .catch(error => {
            console.error('خطأ في جلب تفاصيل السجل:', error);
            showErrorModal('حدث خطأ أثناء جلب تفاصيل السجل');
        });
    }
